...
export const AddRepositoryModal = defineComponent({
  ...
  emits: {
    close: () => true,
    submit: (params: RepoAddParams) => Boolean(params.path.trim()),
  },
  setup: (props, { emit }) => {
    ...
    const closeWhenClickBackdrop = (event: MouseEvent) => {
    ...
    return () =>
      (
        <Transition name="dash-modal-fade" appear>
          {props.open ? (
            <section
              class="dash-add-repository-backdrop fixed inset-0 flex items-center justify-center bg-black/60 p-8"
              aria-label="添加仓库"
              onClick={closeWhenClickBackdrop}
            >
              <form
                class="dash-add-repository-modal w-full max-w-xl rounded-xl border border-slate-700 bg-slate-900 p-5"
                onSubmit={(event) => {
              ...
              >
                <header class="dash-add-repository-header mb-4 flex items-center justify-between">
                  ...
                  <button
                    class="dash-add-repository-close text-slate-400"
                    type="button"
                    aria-label="关闭添加仓库弹窗"
                    onClick={closeModal}
                  >
                ...
                <input
                  ...
                  autofocus
                  onInput={(event) => {
                    repositoryPath.value = (event.target as HTMLInputElement).value;
                  }}
                />
                <footer class="dash-add-repository-footer mt-5 flex justify-end gap-2">
                  <button
                    class="dash-add-repository-cancel rounded-md px-3 py-2 text-sm text-slate-300"
                    type="button"
                    onClick={closeModal}
                  >
                    取消
                  ...
                  <button
                    class="dash-add-repository-submit rounded-md bg-cyan-600 px-3 py-2 text-sm text-white"
                    type="submit"
                    disabled={!repositoryPath.value.trim()}
                  >
                    添加
                  </button>
                </footer>
